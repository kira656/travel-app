    ...
    <SafeAreaView style={{ flex: 1 }} backgroundColor={darkMode ? '#121212' : '#fff'}>
        ...
        <View style={{ padding: 16 }}>
          {loadingWallet ? <ActivityIndicator /> : (
            <View>
              ...
              <Text style={{ fontSize: 28, marginTop: 8 }}>${wallet?.balance ?? '0.00'}</Text>
              <Text style={{ marginTop: 8, color: darkMode ? '#9CA3AF' : '#6B7280' }}>{wallet?.currency ?? 'USD'}</Text>
      ...
      <Modal visible={showRequestModal} animationType="slide" transparent onRequestClose={() => setShowRequestModal(false)}>
        ...
        <View style={{ backgroundColor: darkMode ? '#222' : '#fff', padding: 16, borderTopLeftRadius: 12, borderTopRightRadius: 12 }}>
          ...
          <TextInput value={reqAmount} onChangeText={setReqAmount} placeholder="Amount" keyboardType="numeric" style={{ borderWidth: 1, borderColor: '#e2e8f0', padding: 8, marginTop: 12 }} />
          <TextInput value={reqNote} onChangeText={setReqNote} placeholder="Note (optional)" style={{ borderWidth: 1, borderColor: '#e2e8f0', padding: 8, marginTop: 12 }} />
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12 }}>
            <Pressable onPress={() => setShowRequestModal(false)} style={{ padding: 12 }}><Text style={{ color: '#0a7ea4' }}>Cancel</Text></Pressable>
            <Pressable onPress={submitRequest} style={{ backgroundColor: '#0a7ea4', padding: 12, borderRadius: 8 }}><Text style={{ color: '#fff' }}>Submit</Text></Pressable>
          </View>
        </View>
      </Modal>
